...
Pour cela l'équipe de rOpenSci crée et révise des packages[^noterOpen2] qui facilitent la recherche reproductible. Il peut s'agir de packages qui améliorent l'efficacité et la reproductibilité de son flux de travail comme `drake`[^noterOpen2a] mais aussi des outils qui interrogent des bases de données accessibles en ligne via des interfaces spécifiques, un exemples emblématiques de cette catégorie est `taxize`[^noterOpen2b], véritable couteau suisse pour obtenir des informations taxonomiques. rOpenSci anime également un blog[^noterOpen3] sur lequel on trouve des posts présentant les différents packages révisés par cette organisation et des tutoriels pour apprendre à réaliser certaines manipulations de données avec R. Enfin, rOpenSci anime une communauté[^noterOpen4] dynamique et propose des *community calls*, c'est-à-dire des présentations sur certains aspects de R que l'on peut joindre en directe et qui sont par la suite archivées et consultable sur le site[^noterOpen5].
...
45000 à la fin décembre 2018 d'après https://rdrr.io/). Un autre indice est la popularité de R sur la plateforme d'échange *stack overflow* ou les utilisateurs posent des questions auxquels répondent rapidement d'autres utilisateurs. C'est d’ailleurs sur ces deux derniers
indices que le site internet évaluent la popularité des langage de programmation
et en janvier 2018, R était classé 12^ème^.
...
Un dernier indice de l'accroissement de la popularité de R chez les écologistes que nous
sommes est la forte demande en formation de nos collègues. R figure incontestablement parmi les logiciels les plus utilisés pour la statistics et la science des données de manière générale et actuellement et de plus en plus d'écologue s'y forme à des niveaux de de plus en plus avancés.
...
Tous les acteurs décrits précédemment produisent une grande quantité de resources que ce soit pour débuter avec R, réaliser des tâches spécifiques avec R (au moyen de packages dédiés) mais aussi
pour programmer efficacement avec R et développer ses propres packages[^noteDoc].

Pour débuter avec R, un moyen simple est de consulter les sections *Manuals*, *Books* de la page Internet du projet R (https://www.r-project.org), et la  la section *Contributed* avec des introductions a R dans différentes langues. Cette page n'est malheureusement plus mise à jour mais on y trouve par exemple l'*Introduction à R* de Julien Barnier http://alea.fr.eu.org/pages/intro-R. Il y aussi des acteurs de formation très populaires qui offrent des formations dédiés à R. On peut citer la formation d'introductiona à R de DataCamp (https://www.datacamp.com/courses/free-introduction-to-r, consulté le 28 décembre 2018) mais aussi celle de OpenClassrooms (en français, malgré le nom,   https://openclassrooms.com/fr/courses/1393696-effectuez-vos-etudes-statistiques-avec-r, consulté le 28 décembre 2018).

De la documentation en ligne, gratuite et de qualité est aussi disponible pour se former à des tâches plus  spécifiques. Ainsi, le site bookdown.org[^noteDoc2] propose des livres disponibles en ligne d'une très grande qualité. Notons RStudio référence des ouvrages (certain aussi sur bookdown) https://www.rstudio.com/resources/training/books/. RStudio propose aussi des *cheatsheet* de très bonne uaalité https://www.rstudio.com/resources/cheatsheets/. Les utilisateurs anglophones ont facilement accès à une masse considérable d'informations grâce à différents ouvrages, notamment ceux regroupés dans la collection _Use R!_, publiée par le groupe éditorial Springer et éditée par R. Gentleman, K. Hornik Kurt et G. Parmigiani. Des ouvrages dédiés à R sont aussi disponibles en français, mais en nombre plus restreint. Gaël Millot a ainsi écrit un ouvrage très complet (plus de 800 pages dans la troisième édition), *Comprendre et réaliser les tests statistiques à l'aide de R* édité chez de Boeck. Springer a édité durant un temps des ouvrages dans la série _Pratique R_. Il existe aussi trois ouvrages qui abordent la statistique avec R aux Presses Universitaires de Rennes, ou encore _R et espace_ édité chez Framabook [@groupe_elementr_r_2014].



Une fois les bases acquises avec R, les utilisateurs peuvent avoir recours à la documentation disponible sur des forums tel que celui maintenu par le Centre de coopération internationale en recherche agronomique pour le développement (CIRAD) http://forums.cirad.fr/logiciel-R/index.php. Notons qu'il y a aussi de très bon blogs en anglais http://www.r-bloggers.com, http://www.r-statistics.com, http://blog.rstudio.org, http://blog.revolutionanalytics.com, etc.
...
[^noteDoc2]: https://bookdown.org/  consulté le 28 décembre 2018.
  ...
  différents niveau. Dand des explications, en anglais pour la plupart, pour comprendre ce qu'est R, des liens pour se documenter et connaître les prochains événements autour de R, dont les conférences internationales _Use R!_[^resR1].
...
Blog et twitter iy a des comptes ootuu
...
Pour l'utilisation d'un package spécifique se référer à ;a docuemntation. Nous dévelopons ci dessous.
