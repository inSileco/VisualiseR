...
Le _The R journal_, que nous avons mentionnés plus haut[^noteDoc6], est une revue bisannuelle dédié à l'actualité de R. la majeur partie d'un numéro sont des articles qui présentent les possibilités offertes par certains packages et aussi des synthèse des différentes packages sur un thème précis. On y trouve également un rapport exhaustif détaillant les modifications apportées aux nouvelles versions de R ainsi que les changements sur le CRAN.
...
Enfin une dernière manière de s'informer sur R est de participer à l'une des conférences sur R où se retrouvent les différents acteurs.
La R foundation soutient activement deux conférences *Use R!* et
*DSC* (Directions in Statistical Computing)[^noteDoc7]. La première est présentée comme un forum pour la communauté des utilisateurs de R. La seconde est orientée sur la recherche en statistique computationnelle et donc ne porte pas spécifiquement sur R et plutôt destinée à un public de chercheurs. De plus, RStudio organise chaque année la *rstudio::conf*[^noteDoc8], rOpenSci à sa réunion annuelle, la *unconference* (littéralement la non conférence). Enfin il y a des conférence qui ne sont pas l'initiative de ses acteurs mais de membres actif de la communauté R comme par exemple *R é Québec*[^noteDoc9].
