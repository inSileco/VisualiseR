...
Tous les acteurs décrits précédemment produisent une masse de resources: pour débuter avec R, réaliser des tâches spécifiques avec R (au moyen de packages dédiés) mais aussi
pour programmer efficacement avec R et développer ses propres packages.

Pour débuter avec R, un moyen simple est de consulter les resources pour apprendre et se perfectionner avec R sont abondantes. Sur la page Internet du projet R (https://www.r-project.org), on trouve les sections *Manuals*, *Books* et *The R Journal* qui offrent un grand nombre de resources et sur le CRAN il y a également la section *Contributed* avec des introductions a R dans différentes langues (cette page n'est malheureusement plus mis à jour). Pour ce qui est du français, on y trouve notamment le manuel _R pour les débutants_ d'Emmanuel Paradis, mais aussi un _Aide mémoire R_ rédigé par Mayeul Kauffmann [ref]. Deux autres sources d'informations sont à mentionner bien qu'exclusivement rédigées en anglais.


Certain acteurs propose des formation
...
RStudio offre un grand nimbre de resrouces avec des documentations très bien
faites pour leur packages (par exemple...) et de nombreux ouvrages
d'ailleurs l'un des devlopeurs de RStudio a travaillé sur bookdown sur lesquels
plusieurs livres accessibles en ligne et d'une très grande qualité (XXX).

Au lien _Contributed_ de la section _Documentation_ du CRAN, sont entreposés des manuels d'utilisation de R rédigés par des utilisateurs de R sous différentes langues.
...
voir https://insileco.github.io/wiki/usefulr/
