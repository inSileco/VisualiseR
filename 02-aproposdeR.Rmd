...
Les premières phrases de l'article consacré à R sur Wikipedia définissent R ainsi:
...
Concrètement R est un ensemble de fichiers. Dans la version 3.5.2, il y entre 559 fichiers `.c`, 289 fichiers `.po` (pour la traduction en différente langues), 196 fichiers `.h` (headers utilisé avec C), 86 fichiers `.f` (FORTRAN) et 2346 fichiers `.R`. À l'installation de R, tous ces fichiers sont
compilés de sorte que vous pouvez utiliser R dans un terminal.

![](img/chap2/screenshotR.png)

Un terminal avec R est rarement suffisant pour un développement efficace. C'est pourquoi on utilise une interface de développement, R propose une telle interface mais la plus populaire de nos jours est celle de RStudio, nous y reviendrons.
...
Il y a aujourd'hui une myriade d'acteurs et de sites Internet qui gravitent autour de R de sorte qu'il est parfois un peu difficile de s'y retrouver et ce comprendre qui fait quoi. Ce qui suit est une tentative de mettre un peu d'ordre dans toutes ces informations.


#### Le projet R

Le plus simple pour essayer de se repérer est d'aller sur la page du projet R: https://www.r-project.org/ (consulté le 23 décembre 2018) mais aussi de taper
`contributors()` dans une console R (comme le suggère le message d’accueil).

Sur la page internet du projet R, on trouvera une référence à la Fondation R
https://www.r-project.org/foundation/ (consulté le 23 décembre 2018) et été créé par les membres de la *R Development Core Team* pour pérenniser le développement de R. Les membres de la fondations sont des personnes reconnus pour les efforts non-monétaires qu'ils ont mis pour améliorer R. Quand aux membres de la *R Development Core Team*, ce sont des un groupe de développeurs qui ont un accès d'écriture au code source de R (et les premiers contributeurs listés dans `contributors()`). Ils fixent les bogues dans les fonctionnalités de base de R[^noteR9] et rendent disponibles les nouvelles version de R sur le CRAN.


#### Le CRAN

Le *Comprehensive R Archive Network* (CRAN) est le site Internet où sont archivées les différents versions de R ainsi que les packages qui ont été soumis au CRAN et qui ont été acceptés. C'est aussi une source privilégiée de resource avec de nombre manuels et ouvrage disponible (voir les sections *Manuals* et *Contributed* sur https://cran.r-project.org/). Le CRAN est maintenu par une équipe de volontaires, la "CRAN team", qui mettent à jour le site et s'occupent des soumission des packages. En fait le CRAN est une collection de sites miroirs dont les contenus sont synchronisés avec le site du CRAN maintenu à la Wirtschaftsuniversität à Vienne. En 2002, il existait 12 sites miroirs, et en 2018, on en dénombre plus de 100 qui se répartissent les téléchargements de R et de ses packages, ainsi que l'archivage des différentes versions.


[^noteR9]: Que les utilisateurs peuvent https://bugs.r-project.org/bugzilla3/

[^noteR10]: En 2002, il existait 12 sites miroirs; en 2018, on en dénombre plus de 100 qui se répartissent les téléchargements de R et de ses packages, ainsi que l'archivage des différentes versions.


#### R consortium

Il s'agit d'un projet supporté la la Linux fondation https://www.linuxfoundation.org/projects/ (consulté le 22 décembre 2018)
qui a pour but de supporté le développement de R, ils travaillent de concert avec la R fondation et est supporté notament financer des projets.
Pour voir les soutiens financiers rendez vous sur cette page https://www.r-consortium.org/members

#### R Studio

Vraisemblablement l'un des acteurs les plus connus. C'est une entreprise
fondé par JJ Allaire qui compte en décembre 2018 près d'une centaine de
https://www.rstudio.com/about/. R Studio est connu pour son interface de
et aussi pour la qualité des packages R qu'elles développent avec une équipe de développeurs parmi les plus actifs.


#### R OpenSci

Supporté par NUMFOCUS https://numfocus.org/ https://ropensci.org/about/
...
Bien sur la myriade des utilisateurs de R. Répondent aux questions sur Stack
